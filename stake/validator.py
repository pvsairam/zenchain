...
def bond_with_payee_address(value, payee, commission_rate=0, blocked=False):
    ...
    if check_bonded(MY_ADDRESS):
        print(f"{GREEN}Address {MY_ADDRESS} is already bonded and registered as a validator.{RESET}")
    else:
        ...
        try:
            ...
            print(f"{GREEN}Please wait, preparing for validator activation...{RESET}")
            time.sleep(10)
        except Exception as e:
            print(f"{RED}Bond transaction failed: {e}{RESET}")
            return  # Return if the transaction fails

        # Activate validator with commission rate
        ...
        try:
            ...
            print(f"{RED}Activation transaction failed: {e}{RESET}")

# Example usage
commission_rate = 50000000  # 5% commission
blocked = False
stake_amount = 2  # Amount to stake in ZCX
custom_payee_address = MY_ADDRESS  # Payee address

# Call the bond_with_payee_address function
bond_with_payee_address(stake_amount, custom_payee_address, commission_rate, blocked)
